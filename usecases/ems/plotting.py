...
def compute_mean_epoch_duration(log_dir):
    """
    Compute the mean epoch duration on all the set of experiments contained in the 'log_dir' directory.
    :param log_dir: str; the main logging directory that contains all the experiment related to one of the approaches.
    :return:
    """

    assert os.path.isdir(os.path.join(log_dir)), "log_dir must be the path to a directory"

    # Keep track of all the epoch durations
    epoch_time_vals = list()

    # List all the directories in the main logging directory
    for f in os.listdir(log_dir):
        ...
        if os.path.isdir(path):
            ...
            assert os.path.exists(log_file), "The directory must contain a debug.log file"

            # Read the debug.log file
            with open(log_file) as txt_file:
                lines = txt_file.readlines()
                # Select the only lines with the 'EpochTime' keyword
                epoch_time_lines = [l for l in lines if 'EpochTime' in l]
                # Split the line in two parts: before and after the 'EpochTime' keywork
                for l in epoch_time_lines:
                    tokens = l.split('EpochTime')
                    # The second part contains the epoch duration
                    tokens = tokens[1].split(' ')
                    # The second element is the epoch duration
                    epoch_time_vals.append(float(tokens[1]))

    # Save the epoch durations as a numpy.array
    epoch_time_vals = np.asarray(epoch_time_vals)
    np.save(os.path.join(log_dir, 'mean-epoch-durations'), epoch_time_vals)

########################################################################################################################


def cost_over_time(indexes: np.array,
                   durations_filepath: str,
                   models_filepath: str,
                   acronym: str):
    """
    ...
    rl_true_cost_durations = np.load(os.path.join(durations_filepath, 'rl.npy'))
    rl_safety_durations = np.load(os.path.join(durations_filepath, 'rl-safety-layer.npy'))
    ...
    rl_true_cost_epoch_duration = np.mean(rl_true_cost_durations)
    rl_safety_epoch_duration = np.mean(rl_safety_durations)

    print(f'[Epoch duration] - Single step: {single_step_epoch_duration} | ' +
          f'MDP: {mdp_epoch_duration} ' +
          # f'| RL: {rl_epoch_duration}' +
          f'| RL: {rl_true_cost_epoch_duration}',
          f'| RL safety layer: {rl_safety_epoch_duration}')
    ...
    for i, idx in enumerate(indexes):
        ...
        rl = pd.read_csv(os.path.join(models_filepath,
                                      ...
                                      f'mdp-env_{idx}',
                                      'progress.csv'))
        rl_safety = pd.read_csv(os.path.join(models_filepath,
                                             'rl-safety-layer',
                                             f'mdp-env_{idx}',
                                             'progress.csv'))
        ...
        rl_safety = -rl_safety['Extras/EpisodeRewardMean']
        rl_true_cost = rl['Evaluation/BatchAvgTrueCost']
        rl_termination_rate = rl['Evaluation/TerminationRate']

        # Find the maximum reward to compute normalization
        # all_reward_mean = np.concatenate((single_step, mdp, rl, rl_safety), axis=0)
        all_reward_mean = np.concatenate((single_step, mdp), axis=0)
        ...
        rl_safety = rl_safety / min_reward_mean
        rl_true_cost = rl_true_cost / min_reward_mean
        ...
        if i == 0:
            ...
            normalized_rl_safety = np.expand_dims(rl_safety, axis=0)
            normalized_rl_true_cost = np.expand_dims(rl_true_cost, axis=0)
            ...
            rl_true_cost = np.expand_dims(rl_true_cost, axis=0)
            rl_safety = np.expand_dims(rl_safety, axis=0)
            ...
            normalized_rl_true_cost = np.concatenate((normalized_rl_true_cost, rl_true_cost), axis=0)
            normalized_rl_safety = np.concatenate((normalized_rl_safety, rl_safety), axis=0)
    ...
    normalized_rl_true_cost = np.mean(normalized_rl_true_cost, axis=0)
    normalized_rl_safety = np.mean(normalized_rl_safety, axis=0)
    ...
    rl_true_cost_time = np.arange(len(normalized_rl_true_cost)) * rl_true_cost_epoch_duration
    # rl_time = np.arange(len(normalized_rl)) * rl_epoch_duration
    rl_safety_time = np.arange(len(normalized_rl_safety)) * rl_safety_epoch_duration

    # Find the minimum duration so that rewards array have the same durations
    # min_duration = min(single_step_time[-1], mdp_time[-1], rl_time[-1], rl_safety_time[-1])
    min_duration = min(single_step_time[-1], mdp_time[-1], rl_true_cost_time[-1])
    print(f'[Training end] - Single step: {single_step_time[-1]} ' +
          f'| MDP: {mdp_time[-1]} ' +
          f'| RL: {rl_true_cost_time[-1]}' +
          # f'| RL: {rl_time[-1]}' +
          f'| RL safety layer: {rl_safety_time[-1]}')
    print(f'[Epochs] - Single step: {min_duration / single_step_epoch_duration} ' +
          f'| MDP: {min_duration / mdp_epoch_duration} ' +
          f'| RL: {min_duration / rl_true_cost_epoch_duration}' +
          # f'| RL: {min_duration / rl_epoch_duration}' +
          f'| RL safety: {min_duration / rl_safety_epoch_duration}')
    ...
    rl_true_cost_time = rl_true_cost_time[rl_true_cost_time <= min_duration]
    normalized_rl_true_cost = normalized_rl_true_cost[:len(rl_true_cost_time)]
    rl_termination_rate = rl_termination_rate[:len(rl_true_cost_time)]
    # rl_time = rl_time[rl_time <= min_duration]
    # normalized_rl = normalized_rl[:len(rl_time)]
    rl_safety_time = rl_safety_time[rl_safety_time <= min_duration]
    normalized_rl_safety = normalized_rl_safety[:len(rl_safety_time)]

    # Since the RL method is the most dense array, we perform interpolation for the other methods
    normalized_single_step = np.interp(rl_true_cost_time, single_step_time, normalized_single_step)
    normalized_mdp = np.interp(rl_true_cost_time, mdp_time, normalized_mdp)
    ...
    normalized_rl_safety = np.interp(rl_true_cost_time, rl_safety_time, normalized_rl_safety)

    # Plots the results

    # Figure 1: Model Parameter Tuning

    fig, ax = plt.subplots(1, 1)
    ax.axhline(normalized_robust_kkt,
               label='tuning',
               linestyle=':',
               color='red')
    ax.axhline(normalized_oracle,
               label='oracle',
               linestyle='--',
               color='darkblue')
    ax.plot(rl_true_cost_time,
            normalized_mdp,
            label=f'{acronym}-sequential',
            linestyle='-.',
            color='orange')
    ax.plot(rl_true_cost_time,
            normalized_single_step,
            label=f'{acronym}-single-step',
            linestyle='solid',
            color='green')
    ax.set_xlabel('Time (sec)')
    ax.legend()
    plt.savefig('offline-online-integration', dpi=1000)
    plt.show()

    # Figure 2: Constrained RL

    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True)
    plt.subplots_adjust(hspace=0.2)
    ax1.axhline(normalized_oracle,
                label='oracle',
                linestyle='--',
                color='darkblue')
    ax1.plot(rl_true_cost_time,
             normalized_rl_true_cost,
             label='RL',
             linestyle='-.',
             color='red')
    ax1.plot(rl_true_cost_time,
             normalized_rl_safety,
             label='RL-safety-layer',
             linestyle='dotted',
             color='orange')
    ax1.plot(rl_true_cost_time,
             normalized_mdp,
             ...
             linestyle='solid',
             color='green')
    ax1.set_title('Normalized cost', fontweight='bold')
    ax1.legend()
    ax2.plot(rl_true_cost_time,
             rl_termination_rate,
             label='RL',
             linestyle='-.',
             color='red')
    ax2.legend()
    ax2.set_title('Failed episodes ratio', fontweight='bold')
    ax2.set_xlabel('Time (sec)')

    # plt.title('Mean Episode Reward', fontsize=12, fontweight='bold')
    plt.savefig('constrained-rl', dpi=1000)
    plt.show()
...
if __name__ == '__main__':
    ...
    cost_over_time(indexes=indexes,
                   durations_filepath='results/ems/training-durations/',
                   models_filepath='results/ems/models/',
                   acronym='unify')

    # compute_mean_epoch_duration(log_dir='results/ems/models/rl-safety-layer/')
