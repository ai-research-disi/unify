...
from helpers.online_heuristic import solve
from tabulate import tabulate
from helpers.utility import instances_preprocessing, timestamps_headers, min_max_scaler
from helpers.utility import TIMESTEP_IN_A_DAY, MIN_REWARD
...
class SingleStepVPPEnv(VPPEnv):
    ...
    def step(self, action: np.array) -> Tuple[np.array, Union[float, int], bool, dict]:
        ...
        return observations, reward, done, {'feasible': feasible, 'true cost': -reward}
...
class MarkovianRlVPPEnv(VPPEnv):
    ...
    def __init__(self,
        ...
        :param c_grid: numpy.array; c_grid values.
        :param shift: numpy.array; shift values.
        :param safety_layer: bool; True if you want to use the safety layer action correction.
        :param noise_std_dev: float; the standard deviation of the additive gaussian noise for the realizations.
        :param savepath: string; if not None, the gurobi models are saved to this directory.
        """
    ...
    def _solve(self, action: np.array) -> Tuple[bool, Union[int, float]]:
        ...
        if storage_in > self.cap_max - self.storage or storage_out > self.storage or grid_out < 0:
            # If required, use the safety layer for action correction
    ...
    def step(self, action: np.array) -> Tuple[np.array, Union[float, int], bool, dict]:
        ...
        return observations, reward, done, {'feasible': feasible,
                                            'true cost': self.cumulative_cost}
...
def make_env(method,
    ...
    if method == 'unify-sequential':
        ...
        # Garage wrapping of a gym environment
        env = GymEnv(env, max_episode_length=max_episode_length)
    elif method == 'rl-all-at-once':
        # Create the environment
        env = SingleStepFullRLVPP(predictions=predictions,
                                  shift=shift,
                                  c_grid=c_grid,
                                  noise_std_dev=noise_std_dev,
                                  savepath=None)
        # Garage wrapping of a gym environment
        env = GymEnv(env, max_episode_length=max_episode_length)
    elif method == 'rl-sequential':
        # Create the environment
        env = MarkovianRlVPPEnv(predictions=predictions,
                                shift=shift,
                                c_grid=c_grid,
                                noise_std_dev=noise_std_dev,
                                savepath=None,
