...
# NOTE: this is the only change to the original implementation (use a custom logging function)
from usecases.setcover.rl.utility import log_performance
...
class NPO(RLAlgorithm):
    ...
    def train(self, trainer):
        ...
        # NOTE: get the instances from the environment
        assert hasattr(trainer, 'env'), "trainer must have an env attribute"
        assert hasattr(trainer.env, 'env'), "trainer.env must have an env attribute"
        assert hasattr(trainer.env.env, 'train_instances'), "trainer.env.env must have a train_instances attribute"
        assert hasattr(trainer.env.env, 'test_instances'), "trainer.env.env must have an test_instances attribute"

        train_instances = trainer.env.env.train_instances
        test_instances = trainer.env.env.test_instances
        ...
        for _ in trainer.step_epochs():
            ...
            last_return = self._train_once(trainer.step_itr,
                                           ...
                                           # NOTE: instances for the evaluation
                                           train_instances,
                                           test_instances,
    ...
    def _train_once(self, itr, episodes, train_instances, test_instances, start_time=None):
