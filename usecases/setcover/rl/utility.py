...
def _compute_cost_on_benchmark(instances, actions):
    """
    Compute the cost of the action provided by the agent on a benchmark of instances.
    :param instances: list of usecases.setcover.MinSetCover; the benchmark.
    :param actions: numpy.array of int; predicted demands.
    :return: list of float; the cost for each instance of the benchmark.
    """

    benchmark_costs = list()

    for instance, action in zip(instances, actions):
        virtual_msc = copy.deepcopy(instance)
        virtual_msc.demands = action
        problem = MinSetCoverProblem(instance=virtual_msc)
        solution, _ = problem.solve()

        not_satisfied_demands = instance.demands - action

        # Compute true cost and optimality gap
        cost = compute_cost(instance=instance,
                            decision_vars=solution,
                            not_satisfied_demands=not_satisfied_demands)

        benchmark_costs.append(cost)

    benchmark_costs = np.asarray(benchmark_costs)
    benchmark_costs = np.mean(benchmark_costs)
...
def _get_actions(instances, policy, scaler, action_space):
    observables = [instance.observables for instance in instances]
    # FIXME: this is not compatible with ndarray
    observables = np.asarray(observables)
    _, agent_info = policy.get_actions(observables)
    actions = agent_info['mean']
    action_space = action_space
    actions = scaler.inverse_transform(actions)
    actions = np.rint(actions)
    actions = np.clip(actions, a_min=action_space.low, a_max=action_space.high)
...
def log_performance(itr, batch, policy, discount, train_instances, test_instances, prefix='Evaluation'):
    ...
    Args:
        ...
        policy (rl.algos.NPO): The policy to evaluate.
        discount (float): Discount value, from algorithm's property.
        train_instances (list of usecases.setcover.MinSetCover): Training instances.
        test_instances (list of usecases.setcover.MinSetCover): Test instances.
    ...
    for eps in batch.split():
        ...
        assert 'Cost' in eps.env_infos.keys(), "A cost value is expected"

        batch_costs.append(eps.env_infos['Cost'])
        termination.append(
            float(
    ...
    # NOTE: compute the cost on training and test instances
    '''actions = \
        _get_actions(instances=train_instances,
                     policy=policy,
                     scaler=batch.env_spec.scaler,
                     action_space=batch.env_spec.action_space)
    train_benchmark_costs = _compute_cost_on_benchmark(train_instances, actions)
    actions = \
        _get_actions(instances=test_instances,
                     policy=policy,
                     scaler=batch.env_spec.scaler,
                     action_space=batch.env_spec.action_space)
    test_benchmark_costs = _compute_cost_on_benchmark(test_instances, actions)'''
    ...
    with tabular.prefix(prefix + '/'):
        ...
        tabular.record('BatchAverageCost', np.mean(batch_costs))
        # tabular.record('TrainBenchmarkAverageCost', train_benchmark_costs)
        # tabular.record('TestBenchmarkAverageCost', test_benchmark_costs)
